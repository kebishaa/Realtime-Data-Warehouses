...
class KafkaProduceOperator(BaseOperator):
    ...
    def generate_transaction_data(self, row_num):
        customer_ids = [f"C{str(i).zfill(6)}" for i in range(1, self.num_records + 1)]
        account_ids = [f"A{str(i).zfill(6)}" for i in range(1, self.num_records + 1)]
        branch_ids = [f"B{str(i).zfill(6)}" for i in range(1, self.num_records + 1)]
